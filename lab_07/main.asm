...
inc_input_speed proc near                                   ; если бы объявили far, то при записи нужно было бы помещать адрес процедуры не в DX, а в DS:DX (вроде как, хотя какой far, у нас же тут один сегмент...)
    ...
    push cx						    ; pushf можно не делать (хотя регистр флагов будет меняться в процедуре и его надо сохранить), так как в конце старого обработчика прерывания, который я вызываю в конце процедуры с помощью JMP (а не CALL) есть команда IRET, восстанавливающая то состояние регистра флагов, которое было перед вызовом прерывания (моей процедуры)
    push dx
